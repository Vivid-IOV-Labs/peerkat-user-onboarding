const queries = {
  ...
  countAllUserDeleteAuditRequests: 'SELECT COUNT(*) FROM NFT_USERS.PII."USER_DELETION_AUDIT"',

  updateUserDeletionRequest: 'UPDATE USER_DELETION_AUDIT SET DATE_CONFIRMED_FOR_REMOVAL = ? WHERE USER_ID = ?',
  ...
  getAllUserData: `
  SELECT u.*, s.TELEGRAM, s.DISCORD, s.TWITTER, s.INSTAGRAM, s.FACEBOOK 
  FROM NFT_USERS.PII."USER" u
  LEFT JOIN NFT_USERS.PII."USER_SOCIALS" s ON u.USER_ID = s.USER_ID
  LEFT JOIN NFT_USERS.PII."USER_DELETION_AUDIT" d ON u.USER_ID = d.USER_ID
  WHERE d.USER_ID IS NULL
  LIMIT ? OFFSET ?`,

  getUserDataWithSearchParams: `
  SELECT u.*, s.TELEGRAM, s.DISCORD, s.TWITTER, s.INSTAGRAM, s.FACEBOOK 
  FROM NFT_USERS.PII."USER" u
  LEFT JOIN NFT_USERS.PII."USER_SOCIALS" s
